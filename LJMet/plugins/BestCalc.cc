...
int BestCalc::BeginJob(edm::ConsumesCollector && iC){

    std::cout << "["+GetName()+"]: "<< "initializing parameters" << std::endl;

    m_numSubjetsMin = mPset.getParameter<int>("numSubjetsMin");
    m_numDaughtersMin = mPset.getParameter<int>("numDaughtersMin");
    m_jetSoftDropMassMin = mPset.getParameter<double>("jetSoftDropMassMin");
    m_jetPtMin = mPset.getParameter<double>("jetPtMin");
    m_radiusSmall = mPset.getParameter<double>("radiusSmall");
    m_radiusLarge = mPset.getParameter<double>("radiusLarge");
